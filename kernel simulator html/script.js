(() => {
  ...
  const COLORS = {
    canvasBg: 'black',
    rectFill: 'rgba(30,144,255,0.95)',
    rectBorder: 'rgba(10,60,140,0.95)',
    rotatedBorder: 'white',
    centerDot: 'white',
    mouseStart: 'rgba(0,255,0,0.95)',
    mouseEnd: 'rgba(255,0,0,0.95)',
    arrow: 'blue',
    rightOccupied: 'rgba(32,255,64,0.95)',
    rightEmpty: 'rgba(255,32,64,0.90)',
    rightHover: 'rgba(33,150,243,0.95)',
    rightBorder: 'rgba(0,255,0,0.9)',
    hoverStroke: 'white'
  };
  ...
  imageInput.addEventListener('change', evt => {
    ...
    img.onload = () => {
      loadedImage = img;
      rectWInput.value = img.width;
      rectHInput.value = img.height;
      rectWInput.disabled = true;
      rectHInput.disabled = true;
      draw();
    };
  ...
  function draw() {
    ...
    if(loadedImage){
      leftCtx.drawImage(loadedImage, leftOffsetX + border * scaleL, leftOffsetY + border * scaleL, rectW * scaleL, rectH * scaleL);
    } else {
      leftCtx.fillStyle = COLORS.rectFill;
      leftCtx.fillRect(Math.round(leftOffsetX + border * scaleL), Math.round(leftOffsetY + border * scaleL), Math.max(1, Math.round(rectW*scaleL)), Math.max(1, Math.round(rectH * scaleL)));
    }

    leftCtx.strokeStyle = COLORS.rectBorder;
    leftCtx.lineWidth = LINE_WIDTHS.rectBorder;

    leftCtx.strokeRect(Math.round(leftOffsetX) + 0.5, Math.round(leftOffsetY) + 0.5, Math.max(1, Math.round(imgW * scaleL)) - 1, Math.max(1, Math.round(imgH * scaleL)) - 1);
    ...
    leftCtx.beginPath();
    leftCtx.arc(centerX * scaleL + leftOffsetX, centerY * scaleL + leftOffsetY, Math.max(DOT.leftMinPx, scaleL * DOT.centerMultiplier), 0, Math.PI * 2);
    leftCtx.fill();
    ...
    rightCtx.strokeRect(Math.round(minXr * scaleR + rightOffsetX), Math.round(minYr * scaleR + rightOffsetY), Math.round(Wb * scaleR), Math.round(Hb * scaleR));
    ...
    stats.innerHTML = `Original: ${rectW}×${rectH} px | Border: ${border} | Rotated box: ${Wb.toFixed(2)}×${Hb.toFixed(2)} px &nbsp; | &nbsp; Grid: ${numRows}×${numCols} = ${numRows*numCols} &nbsp; | &nbsp; Occupied: ${occupiedCount} &nbsp; | &nbsp; Samples: ${totalSamples}`;
  }
  ...
  leftCanvas.addEventListener('mousemove', evt => {
    ...
    if (ex >= 0 && ex < gridMeta.imgW && ey >= 0 && ey < gridMeta.imgH) {
      ...
      hoverCell = { r: Math.floor((rp.y - gridMeta.minYr) / gridMeta.kernel), c: Math.floor((rp.x - gridMeta.minXr) / gridMeta.kernel) };
